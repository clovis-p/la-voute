  ...
  <div class="flex flex-col gap-8 w-full max-w-sm p-8 md:p-12">
    ...
    <Form v-slot="$form" :resolver="resolver" :initialValues="initialValues" @submit="handleRegister" class="flex flex-col gap-6 w-full">
      <div class="flex flex-col gap-1">
        <InputText name="firstName" type="text" placeholder="Prénom" class="w-full px-3 py-2 shadow-sm rounded-lg" :invalid="$form.firstName?.invalid" />
        <Message v-if="$form.firstName?.invalid" severity="error" size="small" variant="simple">{{ $form.firstName.error?.message }}</Message>
      </div>
      <div class="flex flex-col gap-1">
        <InputText name="lastName" type="text" placeholder="Nom" class="w-full px-3 py-2 shadow-sm rounded-lg" :invalid="$form.lastName?.invalid" />
      ...
      <div class="flex flex-col gap-1">
        <Password name="confirmPassword" placeholder="Confirmer le mot de passe" :toggleMask="true" :feedback="false" input-class="w-full!" :invalid="$form.confirmPassword?.invalid" />
      ...
      <Button type="submit" label="S'inscrire" class="w-full py-2 rounded-lg flex justify-center items-center gap-2" :loading="loading">
        <template #icon>
          <i class="pi pi-user text-base! leading-normal!" />
        </template>
      </Button>
    </Form>
    <Button label="Retour" severity="secondary" class="w-full py-2 rounded-lg flex justify-center items-center gap-2" @click="emit('switch-to-login')" />
  </div>
</template>
...
const schema = z.object({
  firstName: z.string().min(3, 'Le prénom doit contenir entre 3 et 50 caractères.').max(50, 'Le prénom doit contenir entre 3 et 50 caractères.'),
  lastName: z.string().min(3, 'Le nom doit contenir entre 3 et 50 caractères.').max(50, 'Le nom doit contenir entre 3 et 50 caractères.'),
  username: z.string().min(3, 'Le nom d\'utilisateur doit contenir entre 3 et 50 caractères.').max(50, 'Le nom d\'utilisateur doit contenir entre 3 et 50 caractères.'),
  password: z.string()
