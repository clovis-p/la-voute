...
<template>
  <Dialog :visible="props.visible" modal :header="`Partager ${props.file?.name}`" :style="{ width: '30rem' }" @update:visible="emit('close')">
    <InputGroup class="mb-2">
      <InputText :value="downloadUrl" readonly class="w-[25rem]"></InputText>
    ...
    <InputGroup class="mb-2">
      ...
      <ToggleSwitch v-model="publicShare"></ToggleSwitch>
    </InputGroup>
    <div v-if="!publicShare">
      <InputGroup v-for="(username, index) in usernames" :key="index" class="mb-2">
        ...
        <Button icon="pi pi-minus" severity="danger" @click="handleRemoveUser(index)"></Button>
      </InputGroup>
      <InputGroup class="mb-2">
        <Button label="Ajouter un utilisateur" icon="pi pi-plus" severity="secondary" @click="handleAddUser"></Button>
      </InputGroup>
      <InputGroup>
        <Button label="Soumettre" @click="handleSubmit"></Button>
      </InputGroup>
    </div>
    <div v-else>
      <InputGroup>
